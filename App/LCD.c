...
{
   unsigned char p[8];  //data1当前点的数据 
   unsigned char ImageData[60];
   for(int m=0;m<80;m++)
   {
      for(int i=0;i<60;i++)
    ImageData[i] = ImageData_in[i*10+m/8]>>(7 - m%8)&0x01;
    for(int k=0;k<7;k++)
    {
     LCD_Set_Pos(m,0+k);
     p[k]=ImageData[k*8]+ImageData[k*8+1]*2+ImageData[k*8+2]*4+ImageData[k*8+3]*8+ImageData[k*8+4]*16
     +ImageData[k*8+5]*32+ImageData[k*8+6]*64+ImageData[k*8+7]*128;
     LCD_WrDat(p[k]); 
    }
     LCD_Set_Pos(m,0+7);
     p[7]=ImageData[56]+ImageData[57]*2+ImageData[58]*4+ImageData[59]*8;
     LCD_WrDat(p[7]);
...
void LED_Image(unsigned char *ImageData_in)
{
   ...
   {
      for(int i=0;i<60;i++)
    ImageData[i] = ImageData_in[i*80+m];
    for(int k=0;k<7;k++)
    {
     LCD_Set_Pos(m,0+k);
     p[k]=ImageData[k*8]+ImageData[k*8+1]*2+ImageData[k*8+2]*4+ImageData[k*8+3]*8+ImageData[k*8+4]*16
