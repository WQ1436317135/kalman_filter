 ...
 *   init_x - initial x state value 初始状态值
 *   init_p - initial estimated error convariance 初始估计误差协方差
 * @outputs
 * @retval
 */
void kalman1_init(kalman1_state *state, float init_x, float init_p)
{
    ...
    state->p = init_p;
    state->A = 1;//1 系统参数不要改
    state->H = 1;//1 测量系统参数不要改
    state->q = 100;//原来是200   2e2   10e-6;                      /* predict noise convariance 预测噪声协方差*/
    state->r = 1000;//2600太大，1000可以 原来是500   5e2   10e-5;  /* measure error convariance 测量误差协方差*/
}
 ...
 * @inputs
 *   state - Klaman filter structure
 *   z_measure - Measure value 测量值
 * @outputs
 * @retval
 *   Estimated result 估计结果
 */
